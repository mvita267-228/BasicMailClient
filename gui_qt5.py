"""
Графический интерфейс приложения на PyQt5 (версия для Windows 7 32-bit /
Python 3.8.2 — PyQt6 официально не поддерживает ни Windows 7, ни Python
младше 3.9, поэтому здесь используется PyQt5, у которого шире поддержка
старых систем).
...
def run_app():
    ...
    if lang is None:
        ...
        if lang_dialog.exec() != QDialog.Accepted or not lang_dialog.selected_lang:
            return
